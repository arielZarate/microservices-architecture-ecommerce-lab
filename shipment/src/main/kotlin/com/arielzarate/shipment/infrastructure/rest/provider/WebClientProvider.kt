...
class WebClientProvider(
   ...
   //fixme:revisar handlers
    fun <T : Any, R : Any> applyWithBody(
        ...
        // log { info("Calling ${method.name} to ${uri.toURL()} with body ${getBodyPrettyJson(body)}") }.let {
    ...
    fun <R : Any> apply(
        ...
        return webClientMethod(method)
            ...
            .onErrorResume(TimeoutException::class.java) { ex ->
                ...
                wrappedEx.log {
                    error("[$traceId] Call response TIMEOUT ${method.name} ${uri.toURL()}. Response: ${it.error}")
                }
